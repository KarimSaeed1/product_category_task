...
export default new GraphQLScalarType({
    ...
    serialize(value) {
        ...
        if (isNaN(Number(value)))
            throw new Error(`Decimal serialization error: Value is not a valid decimal: ${value}`);
        // Valid Decimal
        return Number(value);
    },
    ...
    parseValue(value) {
        // Invalid Decimal
        if (isNaN(Number(value)))
            throw new Error(`Decimal parsing error: Invalid decimal format: ${value}`);
        // Valid Decimal
